...
namespace MySql.Server
{
    ...
    public class MySqlServer
    {
        ...
        {
            return string.Format("Server=127.0.0.1;Port={0};SslMode=none;Uid=root;Pwd=root;", _serverPort.ToString());
        }
        ...
        {
            return string.Format("Server=127.0.0.1;Port={0};SslMode=none;Uid=root;Pwd=root;Database={1};", _serverPort.ToString(), databaseName);
        }
        ...
        private void extractMySqlFiles()
        {
            try {
                ...
                if (!new FileInfo(_mysqlDirectory + "\\mysqld.exe").Exists) {
                    ...
                    using var mysqld = typeof(MySqlServer).Assembly.GetManifestResourceStream("NeuroSpeech.Eternity.MySqlStorage.Tests.MySqlInstance.mysqld.exe");

                    using var fs = System.IO.File.OpenWrite(_mysqlDirectory + "\\mysqld.exe");
                    mysqld.CopyTo(fs);
                    fs.Flush();
                    using var errormsg = typeof(MySqlServer).Assembly.GetManifestResourceStream("NeuroSpeech.Eternity.MySqlStorage.Tests.MySqlInstance.errmsg.sys");
                    using var fs2 = System.IO.File.OpenWrite(_mysqlDirectory + "\\errmsg.sys");
                    errormsg.CopyTo(fs2);
                    fs2.Flush();
        ...
        public void StartServer()
        {
            ...
            var StartInfo = new ProcessStartInfo(string.Format("\"{0}\\mysqld.exe\"", _mysqlDirectory));
            ...
            try {
                ...
                _process.OutputDataReceived += (s, e) => {
                    if (e.Data?.Contains("MySQL Community Server (GPL)") ?? false)
                    {
                        started = true;
                    }
                    System.Diagnostics.Trace.WriteLine(e.Data);
